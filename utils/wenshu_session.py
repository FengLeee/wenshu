...
# author__ = "lyao"
# version__ = "1.0.1"
# Date: 2017/09/05 15:41

'''获取一个session对象 里面包含必要的cookie'''
import requests
import execjs
...
def getSess():
    ...
    while True:
        try:
            response = sess.get('http://wenshu.court.gov.cn/list/list/', headers=headers,
            ...
            if response.status_code == 503:
                # 服务器出现问题
                print 'the service is bad and response_status_code is %s, wait one minute retry' %(response.status_code)
                time.sleep(60)
                continue
            if response.status_code == 302:
                while True:
                    # 302重定向 需要输入验证码
                    home_yzm = sess.get('http://wenshu.court.gov.cn/waf_captcha/', headers=headers,
                    ...
                    verify_response = sess.get('http://wenshu.court.gov.cn/waf_verify.htm?captcha={}'.format(captcha),
